...
class PictureCut{
    ...
    public function get(){
          ...
          $this->load->library("TVimageManipulation");
          $this->config->set_item('fileName', $this->currentFile);
        ...
        return $this->CI->tvimagemanipulation->Info();
    }

  	public function getCurrentFileSize() { 
  		if($this->CI->tvimagemanipulation == NULL){
  			throw new Exception("Image not instantiated");
  		} else {
  			return $this->CI->tvimagemanipulation->getCurrentFileSize();
  		}
  	} 
    ...
    public function __construct()
    {
     ...
     $this->CI =& get_instance();
     //$this->CI->load->library("TVimageManipulation");
	...
	public function upload(){
		...
		try {								
          ...
          $this->CI =& get_instance();
          $this->CI->config->set_item('fileName', $this->currentFile);
          $this->CI->load->library("TVimageManipulation");
           			
					//$this->TVimageManipulation = new TVimageManipulation($this->currentFile);
					$this->currentWidth        = $this->CI->tvimagemanipulation->getCurrentWidth();
					$this->currentHeight       = $this->CI->tvimagemanipulation->getCurrentHeight();
					$this->currentFileSize     = $this->CI->tvimagemanipulation->getCurrentFilesize();
						
					if($this->enableResize == "true"){

						if(($this->currentWidth > $this->minimumWidthToResize) || ($this->currentHeight > $this->minimumHeightToResize))
						{
							$this->CI->tvimagemanipulation->resize($this->minimumWidthToResize, $this->minimumHeightToResize);
							$this->CI->tvimagemanipulation->save($this->currentFile);
	...
	public function manipulateSize(){
		...
		while($this->currentFileSize >= $this->maximumSize)
		{
			if($Quality["Current"] >= $Quality["Min"]){
				$this->CI->tvimagemanipulation->save($this->currentFile, ($Quality["Current"]));
				$Quality["Current"] -= $Quality["Step"];
			} else {
				if($Resize["CurrentLoop"] <= $Resize["TotalLoop"]){
					$this->CI->tvimagemanipulation->resizePercent((100-$Resize["Percent"]));
					$this->CI->tvimagemanipulation->save($this->currentFile);
					$Quality["Current"] = 100;
					$Resize["CurrentLoop"]++;
				}
	...
	public function crop(){
        ...
        $this->CI->load->library("TVimageManipulation");
				...
				//$this->CI->tvimagemanipulation = new TVimageManipulation($this->currentFile);
				$this->CI->tvimagemanipulation->crop($this->toCropImgX,	$this->toCropImgY,	$this->toCropImgW,	$this->toCropImgH);
				$this->CI->tvimagemanipulation->save($this->currentFile);
				$this->currentFileSize     =$this->CI->tvimagemanipulation->getCurrentFilesize();			
